...
class Boule_coloree(
    ...
    def dessinimage(self, qp):
        ...
        qp.drawImage(QtCore.QRect(10+ self.x+76.2 , self.y + 10 + 76.2,20,20 ), self.image) #10+76.2+51.3+ 90 +621 - self.y,20,20), self.image)   #à tester
...
class Boule_blanche(Boule):  # on définit la classe représentant les boules colorées,
    ...
    def dessinimage(self, qp):
        #qp.drawImage(QtCore.QRect(self.x - 10, self.y - 10, 20, 20), self.image)
        qp.drawImage(QtCore.QRect(10+self.x +72.6 ,self.y + 10 + 76.2,20,20), self.image)   #10+76.2 + 51.3 + 90+ 621- self.y, 20, 20), self.image)

class Plateau(list):  # le plateau est un espace délimité, composé d'une liste de boules
    def __init__(self, l=10, L=10, nb=1, nc=4, k = 0.999, mode = 0):
        super().__init__(self)
        ...
        if self.mode == 1 : #Le joueur choisit où placer chaque boule
            ...
            self.append(Boule_blanche(0.2 * self.be, 0.75 * self.bs, r= self.be * 0.03 / 2.54))
            self.append(Boule_blanche(0.2 * self.be, 0.25 * self.bs, r=self.be * 0.03 / 2.54))
            self.append(Boule_coloree(0.8 * self.be, 0.5 * self.bs, r=self.be * 0.03 / 2.54))
            self.n = 3
...
class Queue ():
    ...
    def dessinimage(self, qp,image = QtGui.QImage("queue_billard2.png")):
        #qp.drawImage(QtCore.QRect(self.x - 10, self.y - 10, 20, 20), self.image)
        qp.drawImage(QtCore.QRect(self.x -150,self.y-35,300,15), image )   #10+76.2 + 51.3 + 90+ 621- self.y, 20, 20), self.image)
